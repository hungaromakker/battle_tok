...
impl ExtrusionOperation {
    ...
    pub fn update(&mut self, drag_distance: f32, manager: &mut BuildingBlockManager, material: u8) -> Vec<u32> {
        ...
        while self.steps.len() < num_steps {
            ...
            let position = self.face_center + self.face_normal * (step_distance - self.step_size * 0.5);
            
            // Size of the new block (thin slice)
            let half_extents = match self.face {
                FaceDirection::PositiveX | FaceDirection::NegativeX => 
                    Vec3::new(self.step_size * 0.5, self.face_size.x * 0.5, self.face_size.y * 0.5),
                FaceDirection::PositiveY | FaceDirection::NegativeY => 
                    Vec3::new(self.face_size.x * 0.5, self.step_size * 0.5, self.face_size.y * 0.5),
                FaceDirection::PositiveZ | FaceDirection::NegativeZ => 
                    Vec3::new(self.face_size.x * 0.5, self.face_size.y * 0.5, self.step_size * 0.5),
            };
...
impl SculptingManager {
    ...
    fn ray_face_intersection(
        ...
        let face_dir = if abs_local.x > abs_local.y && abs_local.x > abs_local.z {
            if local_hit.x > 0.0 { FaceDirection::PositiveX } else { FaceDirection::NegativeX }
        } else if abs_local.y > abs_local.z {
            if local_hit.y > 0.0 { FaceDirection::PositiveY } else { FaceDirection::NegativeY }
        } else {
            if local_hit.z > 0.0 { FaceDirection::PositiveZ } else { FaceDirection::NegativeZ }
        };
        ...
        match &self.selection {
            SelectionType::Face(face_sel) => {
                ...
                if let Some(block) = manager.get_block(face_sel.block_id) {
                    if let Some(extrusion) = ExtrusionOperation::new(
                        block,
                        face_sel.direction,
                        self.extrusion_step_size,
                    ) {
    ...
    ///
    /// Returns IDs of any newly created blocks
    pub fn update_drag(&mut self, current_pos: Vec3, manager: &mut BuildingBlockManager) -> Vec<u32> {
        if !self.enabled {
            return Vec::new();
        }
...
mod tests {
    ...
    fn test_face_direction_opposite() {
        assert_eq!(FaceDirection::PositiveX.opposite(), FaceDirection::NegativeX);
        assert_eq!(FaceDirection::NegativeY.opposite(), FaceDirection::PositiveY);
    }
