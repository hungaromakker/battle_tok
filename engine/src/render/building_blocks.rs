...
impl BuildingBlock {
    ...
    pub fn aabb(&self) -> AABB {
        match self.shape {
            ...
            BuildingBlockShape::Sphere { radius } => AABB {
    ...
    pub fn sdf(&self, world_point: Vec3) -> f32 {
        ...
        match self.shape {
            BuildingBlockShape::Cube { half_extents } => sdf_box(local_point, half_extents),
            BuildingBlockShape::Cylinder { radius, height } => {
                sdf_cylinder(local_point, radius, height)
            }
            BuildingBlockShape::Sphere { radius } => sdf_sphere(local_point, radius),
            BuildingBlockShape::Dome { radius } => sdf_dome(local_point, radius),
            BuildingBlockShape::Arch {
                width,
                height,
                depth,
            } => sdf_arch(local_point, width, height, depth),
            BuildingBlockShape::Wedge { size } => sdf_wedge(local_point, size),
        }
    }
    ...
    pub fn generate_mesh(&self) -> (Vec<BlockVertex>, Vec<u32>) {
        match self.shape {
            ...
            BuildingBlockShape::Cylinder { radius, height } => mesh_cylinder(
                self.position,
                self.rotation,
                radius,
                height,
...
pub fn sdf_arch(p: Vec3, width: f32, height: f32, depth: f32) -> f32 {
    ...
    // Outer block
    let block_d = sdf_box(
        p - Vec3::new(0.0, height * 0.5, 0.0),
        Vec3::new(half_width, height * 0.5, half_depth),
    );
...
fn mesh_cylinder(
    ...
    for i in 0..segments {
        ...
        indices.extend_from_slice(&[top_center_idx, top_ring_start + next, top_ring_start + i]);
    }
...
mod tests {
    ...
    #[test]
    fn test_sphere_sdf() {
        let block = BuildingBlock::new(BuildingBlockShape::Sphere { radius: 1.0 }, Vec3::ZERO, 0);
