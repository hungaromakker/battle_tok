...
A forest of identical trees looks artificial. Variety systems solve this by generating per-instance variations — slightly different sizes, rotations, tints — from a single base asset. The key insight is **deterministic randomness**: each world position maps to a unique seed, which produces a unique variation. This means the world looks consistent across sessions (no visual popping), artists can predict results, and no variety data needs to be stored per instance — just the seed. Different asset categories need different variety profiles: trees vary a lot (organic), structures barely vary (man-made).
...
1. Define `VarietyParams` struct with `Serialize`/`Deserialize` for `.btasset` storage:
   ...
   pub struct VarietyParams {
       pub scale_min: f32,            // minimum uniform scale (e.g., 0.8)
       pub scale_max: f32,            // maximum uniform scale (e.g., 1.2)
       pub scale_y_bias: f32,         // extra Y-axis stretch range (e.g., 0.1)
       pub random_y_rotation: bool,   // full 360° random Y rotation
       pub tilt_max_degrees: f32,     // max tilt from vertical (e.g., 5.0)
       pub hue_shift_range: f32,      // max hue shift in degrees (e.g., 15.0)
       pub saturation_range: f32,     // max saturation deviation (e.g., 0.1)
       pub brightness_range: f32,     // max brightness deviation (e.g., 0.1)
       pub noise_displacement: f32,   // vertex noise amplitude (e.g., 0.02)
       pub noise_frequency: f32,      // vertex noise frequency (e.g., 1.0)
   }
   ```
2. Define `VarietyInstance` struct — the output of generation:
   ...
   pub struct VarietyInstance {
       ...
       pub y_rotation: f32,           // radians
       pub tilt: [f32; 2],            // tilt angles (x, z) in radians
       pub hue_shift: f32,            // color hue offset
       pub saturation_shift: f32,     // color saturation offset
       pub brightness_shift: f32,     // color brightness offset
   }
   ```
...
4. Implement `seed_from_position(world_x: f32, world_z: f32) -> u32`:
   - Convert floats to integer bits, combine with hash mixing
   - E.g., `(x_bits.wrapping_mul(73856093)) ^ (z_bits.wrapping_mul(19349663))`
   - Ensure non-zero output (xorshift32 requires seed != 0)
5. Implement `generate_variety(params: &VarietyParams, seed: u32) -> VarietyInstance`:
...
pub fn seed_from_position(world_x: f32, world_z: f32) -> u32 {
    ...
    let hash = x_bits.wrapping_mul(73856093) ^ z_bits.wrapping_mul(19349663);
    hash.max(1) // xorshift32 requires non-zero seed
}
