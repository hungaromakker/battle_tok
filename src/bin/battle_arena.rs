...
use battle_tok_engine::game::{
    ...
    // Core types
    Vertex,
    calculate_adjacent_block_position,
    calculate_bridge_segments,
    calculate_ghost_color,
    check_block_support,
    check_capsule_aabb_collision,
    check_capsule_hex_collision,
    check_falling_prism_collision,
    create_test_walls,
    determine_hit_face,
    find_snap_position,
    find_unsupported_cascade,
    generate_all_trees_mesh,
    generate_block_preview_mesh,
    generate_bridge,
    generate_elevated_hex_terrain,
    generate_floating_island,
    generate_hex_grid_overlay,
    generate_lava_ocean,
    generate_lava_plane,
    // Mesh generators
    generate_rotated_box,
    generate_sphere,
    generate_trees_on_terrain,
    get_material_color,
    get_terrain_params,
    has_support,
    hex_to_world_position,
    map_key_to_action,
...
impl BattleArenaApp {
    ...
    fn initialize(&mut self, window: Arc<Window>) {
        ...
        point_lights.add_torch([-3.0, bridge_torch_y, -15.0], torch_color, 8.0);
        println!(
            "[US-P2-014] Point lights initialized: {} torches on floating islands",
            point_lights.light_count()
        );
        ...
        println!(
            "[US-P2-014] Particle system initialized with {} ember spawn positions above lava ocean",
            9
        );
    ...
    fn update(&mut self, delta_time: f32) {
        ...
        if self.left_mouse_pressed && self.build_toolbar.visible {
            ...
            if self.build_toolbar.mouse_hold_time >= PICKUP_HOLD_TIME
                && !self.build_toolbar.pickup_in_progress
            {
                ...
                if let Some((block_id, shape, material)) = self.raycast_to_loose_block() {
                    ...
                    if self.build_toolbar.inventory.stash(shape, material) {
                        ...
                        println!(
                            "[Pickup] Stashed block (inventory: {}/{})",
        ...
        if self.first_person_mode {
            ...
            self.player
                .update(&self.movement, self.camera.yaw, delta_time);
    ...
    fn update_falling_prisms(&mut self, delta_time: f32) {
        ...
        self.falling_prisms.retain(|prism| {
            if prism.grounded {
                ...
                println!(
                    "[Physics] Falling prism IMPACTED ground at ({:.1}, {:.1}, {:.1})",
            ...
            for dq in -1..=1 {
                for dr in -1..=1 {
                    for dl in -1..=1 {
                        ...
                        if self
                            .hex_wall_grid
                            .contains(check_coord.0, check_coord.1, check_coord.2)
                        {
    ...
    fn calculate_block_placement_position(&self) -> Option<Vec3> {
        ...
        Some(Vec3::new(
            snapped_x,
            ground + 0.5 + self.build_toolbar.build_height,
            snapped_z,
        ))
    }
    ...
    fn create_bridge(&mut self) {
        ...
        println!(
            "[Bridge] Creating bridge with {} segments, length={:.1}",
            num_segments, length
        );
        ...
        println!(
            "[Bridge] Bridge created between blocks {} and {}",
    ...
    fn update_building_physics(&mut self, dt: f32, do_support_check: bool) {
        ...
        if do_support_check {
            ...
            println!(
                "[Physics] Running support check on {} blocks...",
                block_count
            );
        }
    ...
    fn handle_block_click(&mut self) -> bool {
        if let Some(block_id) = self.raycast_to_block() {
            ...
            if let Some(blocks_to_merge) = self
                .merge_workflow
                .on_block_click(block_id, &self.block_manager)
            {
                // Double-click detected! Merge connected blocks
                println!(
                    "[Merge] Double-click on block {}, merging {} connected blocks",
                    block_id,
                    blocks_to_merge.len()
                );
                ...
                if let Some(merged) = self.merge_workflow.merge_blocks(
                    ...
                    println!(
                        "[Merge] Created merged mesh with {} vertices",
                        merged.vertices.len()
                    );
            ...
            if horizontal_dist < player_radius {
                ...
                if in_vertical_range {
                    ...
                    if horizontal_dist > 0.001 {
                        ...
                        let push_dir = Vec3::new(to_player.x.signum(), 0.0, to_player.z.signum())
                            .normalize_or_zero();
    ...
    fn regenerate_block_mesh(&mut self) {
        ...
        println!(
            "[Build] Mesh updated: {} vertices, {} indices",
            mesh_vertices.len(),
            indices.len()
        );
    }
    ...
    fn rebuild_terrain(&mut self) {
        ...
        println!(
            "[Floating Islands] Rebuilt! {} vertices, {} triangles",
            static_mesh.vertices.len(),
            static_mesh.indices.len() / 3
        );

        // Also regenerate trees (they depend on terrain height)
        self.trees_attacker = generate_trees_on_terrain(ATTACKER_CENTER, 28.0, 0.3, 0.0);
        self.trees_defender = generate_trees_on_terrain(DEFENDER_CENTER, 28.0, 0.35, 100.0);
    ...
    fn render(&mut self) {
        ...
        for projectile in &self.projectiles {
            let sphere = generate_sphere(
                ...
                [0.2, 0.2, 0.2, 1.0],
                4,
            ); // US-019: Reduced from 8 to 4 segments (4.5x fewer triangles)
        ...
        for falling in &self.falling_prisms {
            ...
            let half_size = Vec3::new(
                DEFAULT_HEX_RADIUS * 0.8,
                DEFAULT_HEX_HEIGHT * 0.5,
                DEFAULT_HEX_RADIUS * 0.8,
            );
            let box_mesh =
                generate_rotated_box(falling.position, half_size, falling.rotation, color);
        ...
        for meteor in &self.meteors {
            if meteor.is_alive() {
                ...
                if meteor.velocity.length() > 1.0 {
                    ...
                    for i in 1..4 {
                        let trail_pos =
                            meteor.position + trail_dir * (i as f32) * meteor.size * 0.8;
        ...
        // ============================================
        {
            ...
            if self.build_toolbar.visible
                && self.build_toolbar.show_preview
                && !self.build_toolbar.is_bridge_mode()
            {
                if let Some(preview_pos) = self.build_toolbar.preview_position {
                    ...
                    let outline_vertices: Vec<Vertex> = preview_verts
                        .iter()
                        .map(|v| {
        ...
        if self.terrain_ui.visible {
            if let (Some(ui_pipeline), Some(ui_bg)) = (&self.ui_pipeline, &self.ui_bind_group) {
                ...
                if !ui_mesh.vertices.is_empty() {
                    ...
                    render_pass
                        .set_index_buffer(ui_index_buffer.slice(..), wgpu::IndexFormat::Uint32);
        ...
        if self.build_toolbar.visible && !self.start_overlay.visible {
            if let (Some(ui_pipeline), Some(ui_bg)) = (&self.ui_pipeline, &self.ui_bind_group) {
                ...
                let add_cross_quad = |verts: &mut Vec<Vertex>,
                                      indices: &mut Vec<u32>,
                                      x1: f32,
                                      y1: f32,
                                      x2: f32,
